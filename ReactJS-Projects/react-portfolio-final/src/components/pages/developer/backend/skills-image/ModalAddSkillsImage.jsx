...
const ModalAddSkillsImage = ({ itemEdit, onClose }) => {
  const { store, dispatch } = React.useContext(StoreContext);
  const handleClose = () => dispatch(setIsAdd(false));

  const initVal = {
    skillsimage_image: itemEdit ? itemEdit.skillsimage_image : "",
  };
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit ? `/v1/skillsimage/${itemEdit.skills_aid}` : `/v1/skillsimage`,
        itemEdit ? "put" : "post",
        values
      ),
  ...
  const yupSchema = Yup.object({
    // skillsimage_image: Yup.string().required("Required"),
  });
            ...
            <Formik
              ...
              onSubmit={async (values) => {
                ...
                mutation.mutate({
                  ...values,
                  skillsimage_image:
                    (itemEdit && itemEdit.skillsimage_image === "") || photo
                      ? photo === null
                        ? itemEdit.skillsimage_image
                        : photo.name
                      : values.skillsimage_image,
                });
              }}
            >
              {(props) => {
                  ...
                  <Form className="flex flex-col h-[calc(100vh-110px)]">
                    <div className="grow overflow-y-auto">
                      <div className="input-wrap">
                        {photo ||
                        (itemEdit && itemEdit.skillsimage_image !== "") ? (
                          <img
                            src={
                              photo
                                ? URL.createObjectURL(photo) // preview
                                : itemEdit.skillsimage_image // check db
                                ? devBaseImgUrl +
                                  "/" +
                                  itemEdit.skillsimage_image
                                : null
...
export default ModalAddSkillsImage;
